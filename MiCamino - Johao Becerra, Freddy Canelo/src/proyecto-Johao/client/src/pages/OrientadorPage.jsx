...
const OrientadorPage = () => {
  ...
  return (
    <div style={styles.page}>
      <div style={{...styles.glow, background: '#a78bfa'}} />

      <header style={styles.header}>
        <span style={styles.logo}>MiCamino<span style={styles.dot}>.</span></span>
        <div style={styles.headerRight}>
          {user && (
            <span style={styles.userTag}>{user.name} · <em>Orientador</em></span>
          )}
      ...
      <main style={styles.main}>
        <div style={{...styles.iconBox, borderColor: '#a78bfa20', background: '#a78bfa12'}}>
          <span style={{fontSize: 40}}>🧭</span>
        </div>

        <h1 style={styles.title}>Panel de <span style={{color: '#a78bfa'}}>Orientador</span></h1>
        <p style={styles.sub}>
          Bienvenido/a, <strong>{user?.name || 'Orientador'}</strong>. Este es tu espacio de trabajo.
        </p>

        <div style={styles.grid}>
          <div style={{...styles.card, borderColor: '#a78bfa30'}}>
            <p style={styles.cardLabel}>🏫 Institución</p>
            <p style={styles.cardValue}>{user?.institucion || 'No asignada'}</p>
          </div>
          <div style={styles.card}>
            <p style={styles.cardLabel}>Rol activo</p>
            <p style={styles.cardValue}>Orientador</p>
          </div>
          <div style={styles.card}>
...
const styles = {
  ...
  userTag: { fontSize: 13, color: 'var(--muted)', fontStyle: 'normal' },
  backBtn: {
