...
import { getEstudiantePorPadre } from '../services/padreService';
...
const PadrePage = () => {
  ...
  const cargarEstudiante = async () => {
    ...
    try {
      const res = await getEstudiantePorPadre(user.id);
  ...
  const handleUpdate = (updated) => { loginUser({ ...user, ...updated }); setShowPerfil(false); };
  const handleDelete = () => { logoutUser(); navigate('/'); };
        ...
        {vista === 'home' && (
          <>
            ...
            <div style={s.vinculoBox}>
              <p style={s.vinculoLabel}>🔗 Estado del vínculo</p>
              <p style={s.vinculoCodigo}>Cuenta activa</p>
              <p style={s.vinculoDesc}>
                Si el administrador ya te vinculó con un estudiante, podrás ver su información aquí.
              </p>
            </div>

            {error && <p style={s.error}>{error}</p>}

            <button style={s.primaryBtn} onClick={cargarEstudiante} disabled={loading}>
          ...
          <div style={s.wrap}>
            ...
            <div style={s.perfilCard}>
              ...
              <div>
                ...
                {estudiante.grado && (
                  <span style={s.perfilBadge}>{estudiante.grado}{estudiante.grupo || ''}</span>
                )}
              </div>
            </div>
            ...
            {estudiante.fecha_vinculacion && (
              <div style={s.infoRow}>
                <span style={s.infoIcon}>📅</span>
                <div>
                  <p style={s.infoLabel}>Vinculado desde</p>
                  <p style={s.infoVal}>{new Date(estudiante.fecha_vinculacion).toLocaleDateString('es-CO')}</p>
                </div>
              </div>
            )}
                  ...
                  {resultado.detalle.map((d, i) => (
                    <div key={i} style={{...s.detalleCard, borderColor: d.correcta ? '#34d39930' : '#f8717130'}}>
                      ...
                      <div style={{flex:1}}>
                        ...
                        <p style={s.detalleRespuesta}>Respondió: <em>{d.opcionElegida}</em></p>
...
const s = {
  ...
  vinculoCodigo:{ fontSize:18, fontWeight:700, color:'var(--text)', marginBottom:8 },
  ...
  infoRow:     { display:'flex', gap:14, alignItems:'center', background:'var(--surface)', border:'1px solid var(--border)', borderRadius:12, padding:'16px 20px', marginBottom:12 },
  ...
  subTitle:    { fontSize:17, fontWeight:600, marginBottom:14, marginTop:8 },
